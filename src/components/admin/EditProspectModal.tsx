/**
 * EditProspectModal
 * Modal d'édition/complétion d'une fiche prospect
 * Permet de renseigner les champs manquants avant conversion en candidat
 */

'use client'

import { useState } from 'react'
import { X, User, Mail, Phone, MapPin, GraduationCap, Euro, Calendar, CheckCircle, Loader2 } from 'lucide-react'
...
interface EditProspectModalProps {
  ...
  onClose: () => void
  onSuccess: (updated: { nom: string; prenom: string; dateNaissance: string | null }) => void
}
...
export function EditProspectModal({ prospect, onClose, onSuccess }: EditProspectModalProps) {
  const [formData, setFormData] = useState({
    ...
    formationPrincipale: prospect.formationSouhaitee !== 'Non définie' ? prospect.formationSouhaitee : '',
    modeFinancement: prospect.financement !== 'Non défini' ? prospect.financement : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        formationPrincipale: formData.formationPrincipale || null,
        modeFinancement: formData.modeFinancement || null,
      }

      // Email : n8n utilise le tableau emails[]
      if (formData.email.trim()) {
        payload.emails = [formData.email.trim()]
      }
      if (formData.telephone.trim()) {
        payload.telephones = [formData.telephone.trim()]
      }
        ...
        {!saved && (
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div>
              ...
              <div className="mt-3">
                <label className="block text-xs font-medium text-[rgb(var(--foreground))] mb-1">
                  Date de naissance <span className="text-[rgb(var(--error))]">*</span>
                  <span className="text-[rgb(var(--muted-foreground))] font-normal ml-1">(requise pour le numéro de dossier)</span>
                </label>
                <div className="relative">
                  <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[rgb(var(--muted-foreground))]" />
                  <input
                    type="date"
            ...
            <div>
              ...
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-[rgb(var(--foreground))] mb-1">Formation souhaitée</label>
                  <div className="relative">
                    <GraduationCap className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[rgb(var(--muted-foreground))]" />
                    <select
                      value={formData.formationPrincipale}
                      onChange={(e) => handleChange('formationPrincipale', e.target.value)}
                      className="w-full pl-9 pr-3 py-2 bg-[rgb(var(--secondary))] border border-[rgba(var(--border),0.5)] rounded-lg text-sm text-[rgb(var(--foreground))] focus:border-[rgb(var(--accent))] focus:outline-none"
                    >
                ...
                <div>
                  <label className="block text-xs font-medium text-[rgb(var(--foreground))] mb-1">Mode de financement</label>
                  <div className="relative">
                    <Euro className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[rgb(var(--muted-foreground))]" />
                    <select
          ...
          <div className="p-4 border-t border-[rgba(var(--border),0.3)] bg-[rgb(var(--secondary))]">
            <div className="flex items-center justify-end gap-2">
              <button
                ...
                onClick={onClose}
                disabled={saving}
                className="px-4 py-2 bg-[rgb(var(--card))] hover:bg-[rgba(var(--accent),0.05)] rounded-lg font-medium transition-all text-sm disabled:opacity-50"
              >
                Annuler
              </button>
              <button
                ...
                disabled={saving}
                className="px-5 py-2 bg-[rgb(var(--accent))] text-[rgb(var(--primary))] rounded-lg font-medium hover:bg-[rgb(var(--accent-light))] transition-all text-sm disabled:opacity-50 flex items-center gap-2"
              >
                {saving ? (
                  <>
